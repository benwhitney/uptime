...
export class UptimeService {
	...
	public start() {
		this.logger = new Logger('Uptime');
		
		this.config = new UptimeConfig();
		this.logger.log('Uptime starting up.');
		this.logger.log('Checking internet status every: ' + this.config.updateIntervalSeconds + ' seconds.');
	...
	private checkStateChange(online: boolean) {
		if (this.lastState === null && online) {
			...
			this.logger.log('The internet is up!')
		...
		if (this.lastState !== online) {
			...
			if (online) {
				// offline -> online
				this.outageEndTime = new Date();
				this.logger.log('Internet Outage Ended');
				this.writeOutageEntry();
			} else {
				// online or null -> offline 
				this.logger.log('THE INTERNET IS DOWN!');
